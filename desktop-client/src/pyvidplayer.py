...
class Video:
    def __init__(self, path):
        ...
        if exists(path):
            self.video = MediaPlayer(path, ff_opts={'vn': ''})  # vn option disables audio
    ...
    def get_playback_data(self):
        return {"active": self.active,
                ...
                "volume": 0,  # Dummy value since there is no audio
    ...
    def set_volume(self, volume):
        pass  # No audio, so volume setting is not applicable
